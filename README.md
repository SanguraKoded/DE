...
- **Journey** (after Day 75): the full Dream Roadmap. Nine phases from student to industry leader, your personal evolution timeline, and 122 tickable milestones across Phase 0 (deliverables and the four-week bridge), Phase 1 (consultancy), Phases 2 to 4 (growth), Phases 5 to 8 (empire), skills beyond tech, and daily habits. Progress is saved like the days.
...
All course content is in `assets/data.js`. `window.CURRICULUM` holds the days, videos, steps, resources, quotes and projects. `window.JOURNEY` holds the roadmap (set `owner` to `""` to remove the name from the page heading; the manifesto and quote text can be edited there too).
