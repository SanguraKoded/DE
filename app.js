...
(function () {
  ...
  var S = load();
  function norm(o) {
    return { v: 1, days: o.days || {}, wins: o.wins || [], journey: o.journey || {}, theme: o.theme || 'auto' };
  }
  function load() {
  ...
  function viewOverview() {
    ...
    if (J) {
      root.appendChild(h('section', { class: 'section' },
        h('h2', { text: 'After Day 75, the journey continues' }),
        h('p', { class: 'sub', text: 'The 75 days are Phase 0 of a longer plan. Track the business, skills and habits that come next in the same place.' }),
        h('ul', { class: 'flow', 'aria-label': 'The journey' }, J.chain.map(function (c) { return h('li', { text: c }); })),
        h('div', { class: 'cta-row', style: 'margin-top:18px' }, h('a', { class: 'btn primary', href: '#/journey' }, 'See the full journey'))));
    }
    var q = quoteOfDay();
  ...
  function viewDay(n) {
    ...
    root.appendChild(h('div', { class: 'day-head' },
      ...
      (n === TOTAL && J) ? h('div', { class: 'banner', style: 'margin-top:10px' }, 'Day 75 is the start, not the end. ', h('a', { href: '#/journey/foundation', style: 'color:inherit;font-weight:600' }, 'Open the bridge: what to do in the four weeks after today')) : null));
  ...
  /* ---------------- journey (after Day 75) ---------------- */
  function jsecCounts(sec) {
    var t = 0, d = 0;
    sec.groups.forEach(function (g) { g.items.forEach(function (it) { t++; if (S.journey[it.id]) d++; }); });
    return { t: t, d: d };
  }
  function jTotals() {
    var t = 0, d = 0;
    J.sections.forEach(function (sec) { var c = jsecCounts(sec); t += c.t; d += c.d; });
    return { t: t, d: d };
  }
  function setJ(id, v) { if (v) S.journey[id] = true; else delete S.journey[id]; save(); }
  function jSection(id) { for (var i = 0; i < J.sections.length; i++) if (J.sections[i].id === id) return J.sections[i]; return null; }
  function jTabs(active) {
    var nav = h('nav', { class: 'jtabs', 'aria-label': 'Journey sections' });
    nav.appendChild(h('a', { href: '#/journey', 'aria-current': active === '' ? 'page' : null }, 'The journey'));
    J.sections.forEach(function (sec) { nav.appendChild(h('a', { href: '#/journey/' + sec.id, 'aria-current': active === sec.id ? 'page' : null }, sec.nav)); });
    return nav;
  }
  function detailList(lines) {
    if (!lines || !lines.length) return null;
    return h('ul', { class: 'subs' }, lines.map(function (t) { return h('li', { class: 'pt' }, linkify(t)); }));
  }

  function viewJourney(id) {
    var sec = id ? jSection(id) : null;
    if (id && !sec) return viewNotFound();
    return sec ? viewJourneySection(sec) : viewJourneyHome();
  }

  function viewJourneyHome() {
    var root = h('div', { class: 'wrap' }), done = countDone(), current = done === TOTAL ? 1 : 0, jt = jTotals();
    root.appendChild(h('div', { class: 'page-head' },
      h('h1', { id: 'pageTitle', tabindex: '-1', text: 'Where the 75 days lead' }),
      h('p', { text: (J.owner ? J.owner + '’s master roadmap. ' : 'Your master roadmap. ') + 'Day 75 is Phase 0 of a longer plan. Every milestone below can be ticked off, just like the days.' })));
    root.appendChild(jTabs(''));
    root.appendChild(h('ul', { class: 'flow', 'aria-label': 'The journey in one line' }, J.chain.map(function (c) { return h('li', { text: c }); })));

    root.appendChild(h('section', { class: 'section', style: 'padding-top:34px' },
      h('h2', { text: 'Nine phases, one direction' }),
      h('p', { class: 'sub', text: 'Each phase changes who you are and what you earn. Open a phase to see its checklist.' }),
      h('ol', { class: 'stairs' }, J.phases.map(function (p, i) {
        var now = i === current;
        var extra = null;
        if (p.n === 0) extra = h('p', { class: 'stair-prog', text: done + ' of ' + TOTAL + ' curriculum days done' });
        return h('li', { class: 'stair' + (now ? ' now' : ''), style: '--i:' + i },
          h('div', { class: 'st-l' }, h('span', { class: 'ic', 'aria-hidden': 'true', text: p.icon }), h('span', { class: 'ph', text: 'Phase ' + p.n })),
          h('div', { class: 'st-m' },
            h('h3', null, p.theme, now ? h('span', { class: 'here', text: done === TOTAL ? 'Up next' : 'You are here' }) : null),
            h('p', { class: 'become', text: p.become }),
            h('p', { class: 'ms', text: p.milestones }),
            extra,
            h('a', { href: '#/journey/' + p.section }, 'Open the checklist')),
          h('div', { class: 'st-r' }, h('span', { class: 'rev', text: p.revenue }), h('span', { class: 'tl', text: p.timeline })));
      }))));

    root.appendChild(h('section', { class: 'section' },
      h('h2', { text: 'Your personal evolution' }),
      h('p', { class: 'sub', text: 'From student to industry leader.' }),
      h('ol', { class: 'evo' }, J.evolution.map(function (e) {
        return h('li', null, h('span', { class: 'ew', text: e.when }), h('span', { class: 'ea', text: e.age }), h('strong', { text: e.identity }), h('span', { text: e.what }));
      }))));

    var jump = h('ul', { class: 'jump' });
    J.sections.forEach(function (sec) {
      var c = jsecCounts(sec);
      jump.appendChild(h('li', null, h('a', { href: '#/journey/' + sec.id },
        h('span', { class: 'jn' }, h('span', { 'aria-hidden': 'true', text: sec.icon + ' ' }), sec.title),
        h('span', { class: 'jc', text: c.d + ' of ' + c.t + ' done' }),
        h('span', { class: 'pr', 'aria-hidden': 'true' }, h('i', { style: 'width:' + Math.round(c.d / c.t * 100) + '%' })))));
    });
    root.appendChild(h('section', { class: 'section' },
      h('h2', { text: 'The checklists' }),
      h('p', { class: 'sub', text: jt.d + ' of ' + jt.t + ' journey items done so far.' }), jump));

    if (J.quote) root.appendChild(h('aside', { class: 'qotd' }, h('blockquote', { text: '“' + J.quote.q + '”' }), h('cite', { text: J.quote.after })));
    return root;
  }

  function viewJourneySection(sec) {
    var root = h('div', { class: 'wrap' }), items = [];
    root.appendChild(h('div', { class: 'page-head' },
      h('h1', { id: 'pageTitle', tabindex: '-1', text: sec.title }), h('p', { text: sec.intro })));
    root.appendChild(jTabs(sec.id));
    var barFill = h('i'), cnt = h('p', { class: 'small', style: 'font:400 .9rem var(--mono);color:var(--muted);margin:0 0 6px' });
    root.appendChild(h('div', { style: 'margin:0 0 8px' }, cnt, h('div', { class: 'bar', style: 'margin-bottom:0' }, barFill)));

    sec.groups.forEach(function (g) {
      var head = h('span', { class: 'count' });
      var list = h('ul', { class: 'checklist' });
      g.items.forEach(function (it) {
        var cb = h('input', { type: 'checkbox', 'aria-label': it.title + ' done', onchange: function () { setJ(it.id, cb.checked); refresh(); } });
        var li = h('li', { class: 'step' },
          h('label', null, cb, h('div', { class: 'txt' },
            h('div', { class: 'main' },
              it.when ? h('span', { class: 'when', text: it.when }) : null,
              it.tag ? h('span', { class: 'tag', text: it.tag }) : null,
              h('span', { class: 'jt', text: it.title })),
            detailList(it.detail),
            it.pub ? h('p', { class: 'pub' }, h('b', { text: 'How to publish: ' }), it.pub) : null)));
        items.push({ it: it, cb: cb, li: li, g: g });
        list.appendChild(li);
      });
      g._head = head;
      root.appendChild(h('section', { class: 'jgroup', 'aria-label': g.title },
        h('div', { class: 'block-head' }, h('div', null, h('h2', null, g.icon ? g.icon + ' ' : '', g.title), g.sub ? h('p', { class: 'muted', style: 'margin:4px 0 0', text: g.sub }) : null), head), list));
    });

    if (sec.id === 'habits' && J.manifesto) {
      var lines = J.manifesto.filter(Boolean);
      root.appendChild(h('aside', { class: 'qotd manifesto' },
        h('h2', { text: lines[0].replace(/^🏆\s*/, '') }),
        lines.slice(1).map(function (l) { return h('p', { text: l }); })));
    }

    var idx = J.sections.indexOf(sec), pager = h('nav', { class: 'pager', 'aria-label': 'Previous and next section' });
    var prevHref = idx === 0 ? '#/journey' : '#/journey/' + J.sections[idx - 1].id;
    var prevLabel = idx === 0 ? 'The journey' : J.sections[idx - 1].title;
    pager.appendChild(h('a', { class: 'prev', href: prevHref }, h('small', { text: 'Previous' }), prevLabel));
    if (idx < J.sections.length - 1) pager.appendChild(h('a', { class: 'next', href: '#/journey/' + J.sections[idx + 1].id }, h('small', { text: 'Next' }), J.sections[idx + 1].title));
    root.appendChild(pager);

    function refresh() {
      items.forEach(function (x) { var c = !!S.journey[x.it.id]; x.cb.checked = c; x.li.classList.toggle('done', c); });
      var c = jsecCounts(sec);
      cnt.textContent = c.d + ' of ' + c.t + ' done';
      barFill.style.width = (c.t ? c.d / c.t * 100 : 0) + '%';
      sec.groups.forEach(function (g) {
        var d = 0; g.items.forEach(function (it) { if (S.journey[it.id]) d++; });
        g._head.textContent = d + ' of ' + g.items.length;
      });
    }
    refresh();
    return root;
  ...
  function route() {
    ...
    else if (name === 'journey' && J) { view = viewJourney(parts[1] || ''); title = 'Journey · DE 75'; navKey = 'journey'; }
  ...
  var JINDEX = [];
  if (J) J.sections.forEach(function (sec) { sec.groups.forEach(function (g) { g.items.forEach(function (it) {
    var body = (it.detail || []).join(' ');
    JINDEX.push({ sec: sec, it: it, title: it.title, tl: it.title.toLowerCase(), all: (it.title + ' ' + body + ' ' + g.title + ' ' + (it.when || '') + ' ' + (it.tag || '')).toLowerCase(), body: body });
  }); }); });
  function doSearch() {
    var q = sIn.value.trim().toLowerCase(); sOut.textContent = ''; sSel = -1;
    if (!q) { sOut.appendChild(h('li', { class: 'muted', style: 'padding:10px 8px', text: 'Try “window functions”, “docker”, “kafka”, “resume” or “clients”.' })); return; }
    ...
    var jhits = [];
    JINDEX.forEach(function (x) {
      if (!terms.every(function (t) { return x.all.indexOf(t) >= 0; })) return;
      jhits.push({ x: x, score: terms.every(function (t) { return x.tl.indexOf(t) >= 0; }) ? 0 : 1 });
    });
    jhits.sort(function (a, b) { return a.score - b.score; });
    if (!hits.length && !jhits.length) { sOut.appendChild(h('li', { class: 'muted', style: 'padding:10px 8px', text: 'Nothing matches “' + sIn.value.trim() + '”. Try fewer or different words.' })); return; }
    var room = jhits.length ? 10 : 15;
    hits.slice(0, room).forEach(function (hit) {
    ...
    jhits.slice(0, 15 - Math.min(hits.length, room)).forEach(function (jh) {
      sOut.appendChild(h('li', { role: 'option' }, h('a', { href: '#/journey/' + jh.x.sec.id, onclick: function () { sDlg.close(); } },
        h('span', { class: 'rd', text: 'Journey' }),
        h('span', { class: 'rt' }, highlight(jh.x.title, terms), jh.x.body ? h('span', { class: 'rs' }, highlight(jh.x.body, terms)) : null))));
    });
  }
